...
In Experiment 1a I fixed the activation function for hidden layers to be ReLU and I built models by iterating over hyperparameter space which I generated arbitarily. I found that the best model has following parameters:
  ...
  * momentum start: 0.5
  * Input Scaling: TRUE
  * Accuracy (validation): 0.9863
  * Accuraccy (test): 0.9432
I expected the model to have maximum number of hidden units and layers but this is not the result I found. Infact the model with 3 hidden layers with 300 units and other hyperparameters same as the best model gave an accuraccy of only 45% on the test set.
...
### Experiment 2: Best model confusion matrix and model summary
```{r,tidy=TRUE}
bestModelc
```
...
I found the best convolutional model had the following hyperparameters:
  * Hidden units in layer1
  * Kernel size
  * Number of filters
  * Learning rate
